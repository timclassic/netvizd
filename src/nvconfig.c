...
int nv_validate_conf() {
	...
	if (nv_proto_p_list.next == NULL) {
		nv_log(NVLOG_ERROR, "need at least one protocol plugin defined");
		stat = -1;
	}

	/* check for an auth plugin */
	if (nv_auth_p_list.next == NULL) {
		nv_log(NVLOG_ERROR, "need at least one authentication plugin defined");
		stat = -1;
	}

	/* check for a storage instance */
	if (nv_stor_list.next == NULL) {
		nv_log(NVLOG_ERROR, "need at least one storage instance defined");
		stat = -1;
	}

	/* check for a data set */
	if (nv_dsts_list.next == NULL) {
		nv_log(NVLOG_ERROR, "need at least one data set defined");
		stat = -1;
	}
